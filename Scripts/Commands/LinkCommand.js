...
LinkCommand.prototype.execute = function () {
    ...
    for (var i = 0; i < this.responseText.length; i++) {
        ...
        if (label.indexOf("c:img") > -1)
        {
            ...
            var resultImg = regex.exec(element);
            var urlImg = this.getParameterImg('url', resultImg);
            var widthImg = this.getParameterImg('width', resultImg);
            var heightImg = this.getParameterImg('height', resultImg);

            label = '<img src="' + urlImg + '" width="' + widthImg + '" height="' + heightImg + '" />';
            url = '';
        }
		/* ADD IMG ON C:LINK */
...
/* ADD IMG ON C:LINK */
LinkCommand.prototype.getParameterImg = function (name, responseText) {
    var regex = new RegExp(name + "=([^#=]*)[=|\\]]"), results = regex.exec(responseText);
    return results === null ? "" : results[1].replace(/url|width|height|] value/g, '');
}
/* ADD IMG ON C:LINK */